...
//
// - offer_booking      -> the web widget shows the live in-chat time picker
...
export async function performActions(actions, person) {
  ...
  for (const action of actions ?? []) {
    try {
      switch (action.type) {
        case "offer_booking":
          // No link involved: the web widget shows the live time picker. We just
          // record that a booking was offered this turn.
          done.push("offer_booking");
          break;
...
export async function sendBookingConfirmations({ person, when, bookingType }) {
  ...
  const whenStr = when.toLocaleString("en-GB", { weekday: "long", day: "numeric", month: "long", hour: "numeric", minute: "2-digit", hour12: true, timeZone: tz });
...
export async function replyByEmail({ to, subject, text }) {
  return await sendEmail({ to, subject, text });  // true only if actually sent
}
