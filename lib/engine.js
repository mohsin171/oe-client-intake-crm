...
export async function processTurn({ message, sessionId, channel = "web", source = "" }) {
  ...
  // If a human adviser has taken over this lead, the AI stands down: record the
  // incoming message so the adviser sees it, but do not auto-reply.
  const existing = await getPerson(id);
  if (existing && existing.agent_takeover) {
    await addEvent({ firmId, personId: id, type: "message_while_takeover", detail: `${channel} message received while adviser handling`, actor: "system" });
    return { reply: null, sessionId: id, handledByHuman: true, spam: false, mode: isDemoMode() ? "demo" : "live" };
  }
  ...
  let replyText = reply;
  // When the AI signals booking intent (send_booking_link), tell the web widget
  // to render the live slot picker. The picker fetches real open times from
  // /api/book (already-booked slots are excluded there) and books on tap. Other
  // channels ignore this flag and rely on the AI's text to gather a time.
  const showBooking = done.includes("send_booking_link");
  if (showBooking) {
    await addEvent({ firmId, personId: id, type: "booking_offered", detail: `slot picker offered (${channel})`, actor: "ai" });
  }
  ...
  return { reply: replyText, sessionId: id, bookingType: CONFIG.firm.bookingType, showBooking, spam: false, mode: isDemoMode() ? "demo" : "live" };
}
